...
const getCardBgColor = (tags: string[]) => {
  ...
  if (tags.includes("HIIT")) return "bg-[#C49A6C]";
  return "bg-[#35C7D2]";
};

interface TrialScheduleCardsProps {
  onSelectClass: (classItem: ClassItem, dayKey: string) => void;
}
...
export default function TrialScheduleCards({ onSelectClass }: TrialScheduleCardsProps) {
  ...
  const handleClassAction = (classItem: ClassItem, dayKey: string) => {
  ...
  const renderCard = (classItem: ClassItem, dayKey: string, index: number, showDayAbbr = false) => (
    <div
      key={`${dayKey}-${index}`}
    ...
    >
      ...
      <div className="flex items-center gap-2 text-white/90 text-xl md:text-2xl font-semibold mb-2 pr-12 lg:pr-0">
        ...
        {classItem.instructor && (
          <span className="text-sm font-normal text-white/70 ml-2">con {classItem.instructor}</span>
        )}
      </div>
      ...
      {classItem.description && (
        <p className="text-white/80 text-sm mb-3 line-clamp-2">{classItem.description}</p>
      )}
        ...
        {classItem.is_trial_enabled ? (
          ...
          <>
            <span className="inline-flex items-center gap-1 bg-white/20 text-white/80 rounded-full px-3 py-1 text-xs backdrop-blur">
              ...
              No permite clase de prueba
            </span>
            <button
              onClick={() => handleClassAction(classItem, dayKey)}
              className="ml-auto bg-white/20 text-white font-medium text-sm px-4 py-2 rounded-full hover:bg-white/30 transition-colors backdrop-blur"
            >
              Agendar
            </button>
          </>
            ...
            <div className="flex gap-2 md:gap-3 overflow-x-auto pb-2 -mx-1 px-1 [&::-webkit-scrollbar]:hidden">
              {EXPERIENCE_CATALOG.map(exp => (
                <button
                  ...
                  className={`shrink-0 whitespace-nowrap rounded-full px-4 py-2.5 border transition-all text-sm font-medium ${
                    expSlug === exp.slug
          ...
          <>
            ...
            <h2 className="text-xl md:text-2xl font-bold text-[#2E4D3A] mb-4 flex items-center gap-2">
              ...
              {selectedDay === todayKey && (
                <span className="bg-[#2E4D3A] text-white rounded-full text-sm px-3 py-1">Hoy</span>
              )}
            </h2>
            ...
            >
              {currentDayClasses.map((classItem, index) =>
                renderCard(classItem, getActiveDay(), index, true)
              )}
            ...
            {experienceWeekData.map(({ day, dayName, items }) => (
              <div key={day} className="bg-white rounded-2xl p-6 shadow-sm">
                ...
                <ul className="space-y-3">
                  {items.map((item, idx) => {
                    // Find matching ClassItem from scheduleData
                    const dayClasses = scheduleData[day] || [];
                    const matchedClass = dayClasses.find(c => c.time === item.time && c.title === item.title);
                    const isTrial = matchedClass?.is_trial_enabled ?? false;
                    ...
                    return (
                      <li key={idx} className="flex flex-col gap-2 p-3 bg-[#F8F9FA] rounded-lg">
                        ...
                        {matchedClass?.description && (
                          <p className="text-sm text-[#575757] ml-9">{matchedClass.description}</p>
                        )}
                        <div className="flex items-center gap-2 ml-9 mt-1">
                          {isTrial ? (
                            <>
                              <span className="inline-flex items-center gap-1 bg-emerald-100 text-emerald-700 rounded-full px-2.5 py-0.5 text-xs">
                                <CheckCircle className="w-3 h-3" /> Clase de prueba
                              </span>
                              <button
                                onClick={() => matchedClass && onSelectClass(matchedClass, day)}
                                className="text-sm font-semibold text-[#2E4D3A] underline hover:no-underline"
                              >
                                Agendar prueba
                              </button>
                            </>
